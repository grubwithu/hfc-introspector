...
PreservedAnalyses FuzzIntrospectorPass::run(Module &M,
  ...
  std::cout << "Running FuzzIntrospectorPass\n";
...
llvm::PassPluginLibraryInfo getFuzzIntrospectorPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "FuzzIntrospector", LLVM_VERSION_STRING,
          [](llvm::PassBuilder &PB) {
            // 这里注册它可以被 "opt -passes=fuzz-introspector" 调用
            PB.registerPipelineParsingCallback(
                [](llvm::StringRef Name, llvm::ModulePassManager &MPM,
                   llvm::ArrayRef<llvm::PassBuilder::PipelineElement>) {
                  if (Name == "fuzz-introspector") {
                    // 假设 FuzzIntrospector 是一个 Module Pass
                    // 并且有一个默认构造函数
...
llvmGetPassPluginInfo() {
  return getFuzzIntrospectorPluginInfo();
}